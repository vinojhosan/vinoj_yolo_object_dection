...
ds = dataset.SynthenticGenerator((input_h, input_w), (grid_h, grid_w), batch_size=8, n_images=1000)
...
def get_model():
    ...
    output_channel = len(ds.anchors)*(ds.n_class+1 + 4)
    resnet_connector = k.layers.Conv2D(output_channel, (3, 3), padding='same', activation='relu')(activation_49)
    print(resnet_connector.shape)
    reshaping = k.layers.Reshape([grid_h, grid_w, len(ds.anchors), (ds.n_class + 1 + 4)])(resnet_connector)

    ssd_model = k.models.Model(input_layer, reshaping)
...
def train():
    ...
    yolo_model.compile(adam, loss=ssd_loss.ssd_loss, metrics=[identity_metric])
