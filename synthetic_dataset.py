...
class SynthenticGenerator(Sequence):
    ...
    def __init__(self, image_size, grid, n_images=200, batch_size=8):
    ...
    def __getitem__(self, idx):
        ...
        y_batch = np.zeros((self.batch_size, GRID_H, GRID_W, len(self.anchors), 4 + self.n_class + 1), np.float32)  # desired network output
    ...
    def get_predict2Boxes(self, x_batch, y_batch):
        ...
        for box_itr in range(len(predicted_objects[0])):
            ...
            box = y_batch[b, r, c, a, 0:4]
            class_id = np.argmax(y_batch[b, r, c, a, 4:])
